...
![Editor and footer](./assets/editor.png)
...
- Collapsed `grep` / `find` / `ls` / `bash` keep at most three result lines; consecutive collapsed successful `read`s in the same turn stack into one list; a single `read` stays one unboxed line; `write` and `edit` stay native; `Ctrl+O` expands to Pi's renderer
